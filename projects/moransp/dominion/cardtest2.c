 ...
 * 
 * * To compile from the command line, call 'make cardtest2' and be sure to
...
int main() {
	...
	for (i=1;i<G.numPlayers;i++){
		bugtracker +=expected(0, G.handCount[i], "Hand Count other players:");
		bugtracker += expected(10, G.deckCount[i], "Deck Count other players:");
		bugtracker += expected(0, G.discardCount[i], "Discard Count other players");
	}
printf("%s\n",KNRM);	
return 0;
}
